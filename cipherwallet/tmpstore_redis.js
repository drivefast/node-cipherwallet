...
var redis = Redis.createClient(constants.REDIS_PORT, constants.REDIS_HOST);
...
module.exports = {
    ...
    is_nonce_valid: function (arg1, arg2, ttl, cb) {
        ...
        redis.set(K_NONCE.format(arg1, arg2), 0, ttl, "NX", function(err) { cb(err); });
    },

    cw_session_data: function(session_id, svar, val, cb) {
        // cipherwallet session variables managed in the temp store
        redis.get(K_CW_SESSION.format(session_id), function(err, json_s) {
            if (err) 
               cb(err, null);
            else {
